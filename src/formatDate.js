...
function formatDate(date, fromFormat, toFormat) {
  ...
  for (const name of toFormat) {
    switch (name) {
      case 'YYYY':
        correctFormat.push(dateObject['YYYY']);
        break;
      case 'YY':
        correctFormat.push(dateObject['YYYY'].slice(2));
        break;
      case 'MM':
        correctFormat.push(dateObject['MM']);
        break;
      case 'DD':
        correctFormat.push(dateObject['DD']);
        break;
    }
  }

  return correctFormat.join(toFormat[toFormat.length - 1]);
}
