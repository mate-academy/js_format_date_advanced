...
function formatDate(date, fromFormat, toFormat) {
  ...
  for (let i = 0; i < fromFormat.length - 1; i++) {
    dateObj[fromFormat[i]] = datePrepared[i];

    if (fromFormat[i] === 'YY') {
      if (+datePrepared[i] >= 0 && +datePrepared[i] <= 22) {
        dateObj['YYYY'] = '20' + datePrepared[i];
      } else {
        dateObj['YYYY'] = '19' + datePrepared[i];
      }
    }

    if (fromFormat[i] === 'YYYY') {
      dateObj['YY'] = ('' + datePrepared).slice(-2);
    }
  }
  ...
  return newDate.join(dateObj.separator);
}
